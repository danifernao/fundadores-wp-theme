<?php
    ...
    // Agrega un campo en Identidad del sitio para subir una imagen para la cabecera del formulario de Inicio de sesión.
    function customizer_settings( $wp_customize ) {
        $wp_customize->add_setting('auth_header_img', array(
            'default' => '',
            'sanitize_callback' => 'esc_url_raw',
            'transport' => 'refresh',
        ));
        
        $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'auth_header_img_control', array(
            'label' => __('Imagen de cabecera de la página de Inicio de sesión', 'fundadores'),
            'section' => 'title_tagline',
            'settings' => 'auth_header_img',
        )));
    }
    ...
    function theme_resources_enqueue() {
        ...
        if ( ! empty( $site_key ) ) {
          wp_enqueue_script( 'recaptcha-js', 'https://www.google.com/recaptcha/api.js?onload=onloadCallback' );
        }
    }
    ...
    function login_resources_enqueue() {
        ...
        // Si se ha definido un icono para el sitio web, lo emplea en la página.
        $header_img = get_theme_mod('auth_header_img');
        if ( $header_img  ) {
            $custom_css = "
              .login h1 a {
            ...
            ";
            wp_add_inline_style( 'wp-css', $custom_css );
        }
    }
